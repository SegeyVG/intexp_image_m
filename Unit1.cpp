...
Byte * Mem1; // указатель на вторичный буфер под пиксели из входного файла (после дополнительной обработки, если она есть)
...
void __fastcall TForm1::OpenImage1Click(TObject *Sender) {
	if (OpenPictureDialog1->Execute()) {
		...
		if (f) {
			...
			PutMem1toMem2();
			for (int i = 0; i < H;
			i++) // подготовка изображения к выводу на форму
			{
